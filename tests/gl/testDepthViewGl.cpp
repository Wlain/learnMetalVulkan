...
class TestDepthViewGl : public EffectBase
{
public:
    using EffectBase::EffectBase;
    ~TestDepthViewGl() override = default;
...
void testDepthViewGl()
{
    Device::Info info{ Device::RenderType::OpenGL, 800, 600, "OpenGL Example Depth View" };
    DeviceGl handle(info);
    handle.init();
    GLFWRendererGL rendererGl(&handle);
    Engine engine(rendererGl);
    auto effect = std::make_shared<TestDepthViewGl>(&rendererGl);
