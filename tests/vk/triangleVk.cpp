...
public:
    ...
    void buildBuffers()
    {
        m_vertexMesh = MAKE_SHARED(m_vertexMesh, m_deviceVk);
        m_vertexMesh->create(g_quadVertex.size() * sizeof(g_quadVertex[0]), (void*)g_quadVertex.data(), Buffer::BufferUsage::StaticDraw, Buffer::BufferType::VertexBuffer);
        m_indexMesh = MAKE_SHARED(m_indexMesh, m_deviceVk);
        m_indexMesh->create(g_quadIndices.size() * sizeof(g_quadIndices[0]), (void*)g_quadIndices.data(), Buffer::BufferUsage::StaticDraw, Buffer::BufferType::IndexBuffer);
    }
    ...
    void render() override
    {
        ...
        for (std::size_t i = 0; i < commandBuffers.size(); ++i)
        {
            ...
            auto vertexBuffers = std::array<vk::Buffer, 1>{ m_vertexMesh->buffer() };
            auto offsets = std::array<vk::DeviceSize, 1>{ 0 };
            commandBuffers[i].bindVertexBuffers(0, 1, vertexBuffers.data(), offsets.data());
            commandBuffers[i].bindIndexBuffer(m_indexMesh->buffer(), 0, vk::IndexType::eUint16);
...
private:
    ...
    std::shared_ptr<BufferVK> m_vertexMesh;
    std::shared_ptr<BufferVK> m_indexMesh;
