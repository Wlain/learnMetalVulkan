...
namespace
{
class TestDepthTestMtl : public EffectBase
{
public:
    using EffectBase::EffectBase;
    ~TestDepthTestMtl() override
    {
...
void testDepthTestMtl()
{
    Device::Info info{ Device::RenderType::Metal, 800, 600, "Metal Example DepthTest" };
    DeviceMtl device(info);
    device.init();
    GLFWRendererMtl rendererMtl(&device);
    Engine engine(rendererMtl);
    auto effect = std::make_shared<TestDepthTestMtl>(&rendererMtl);
