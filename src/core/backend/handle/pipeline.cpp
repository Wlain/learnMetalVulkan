...
std::pair<std::vector<uint32_t>, std::vector<uint32_t>> Pipeline::getSpvFromGLSL(std::string_view vertexSource, std::string_view fragSource)
{
    ...
    bool vertexResult = vertexShader.parse(&glslang::DefaultTBuiltInResource, 330, ECoreProfile, false, false, messages);
    if (!vertexResult)
    {
    ...
    bool fragmentResult = fragmentShader.parse(&glslang::DefaultTBuiltInResource, 330, ECoreProfile,
